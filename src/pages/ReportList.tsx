...
const ReportList = () => {
  ...
  const { user } = useAuth(); // 2. Ambil data user login
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      if (startDateFilter) query = query.gte('start_date', startDateFilter);
      if (endDateFilter) query = query.lte('start_date', endDateFilter);
      if (searchText) query = query.ilike('problem_description', `%${searchText}%`);
    ...
    statusFilter, equipmentFilter, shiftFilter, startDateFilter, endDateFilter, 
    searchText, sortKey, sortOrder, page, pageSize
  ]);

  // Frontend search filtering (secondary)
  useEffect(() => {
    let result = rawData;
    if (searchText) {
  ...
  // Build Options
  const equipmentOptions = Array.from(new Set(rawData.map(d => d.equipment?.code && d.equipment.id ? `${d.equipment.id}__${d.equipment.code}` : null)))
    .filter(Boolean)
    .map(str => {
  ...
  const shiftOptions = Array.from(new Set(rawData.map(d => d.shift)))
    .filter(Boolean)
    ...
    },
    {
      ...
      cell: ({ row }: any) => (
        <div className="flex gap-2">
          {/* 3. LOGIC HIDE EDIT BUTTON UNTUK MECHANIC */}
          {user?.role !== 'mechanic' && (
    ...
    <div className="p-4 space-y-4">
      <div className="flex flex-col sm:flex-row justify-between gap-2">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-2">
          <input type="text" placeholder="Search problem" value={searchText} onChange={(e) => { setSearchText(e.target.value); setPage(1); }} className="border p-2 rounded" />
          ...
          <select value={equipmentFilter} onChange={e => { setEquipmentFilter(e.target.value); setPage(1); }} className="border p-2 rounded">
            <option value="">All Equipment</option>
            {equipmentOptions.map(opt => ( <option key={opt.value} value={opt.value}>{opt.label}</option> ))}
          </select>
          <select value={shiftFilter} onChange={e => { setShiftFilter(e.target.value); setPage(1); }} className="border p-2 rounded">
            <option value="">All Shift</option>
            {shiftOptions.map(opt => ( <option key={opt.value} value={opt.value}>{opt.label}</option> ))}
          </select>
          <input type="date" value={startDateFilter} onChange={e => { setStartDateFilter(e.target.value); setPage(1); }} className="border p-2 rounded" placeholder="Start Date" />
          <input type="date" value={endDateFilter} onChange={e => { setEndDateFilter(e.target.value); setPage(1); }} className="border p-2 rounded" placeholder="End Date" />
        </div>
      ...
      <div className="flex items-center gap-4 my-2">
        <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="border px-2 py-1 rounded">Prev</button>
        <span>Page {page} of {Math.max(1, Math.ceil(total / pageSize))}</span>
        <button onClick={() => setPage((p) => (p * pageSize < total ? p + 1 : p))} disabled={page * pageSize >= total} className="border px-2 py-1 rounded">Next</button>
