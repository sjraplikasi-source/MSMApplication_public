...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div
        // --- PERUBAHAN 1 ---
        // Tambahkan kondisi !isMobileOpen pada event hover
        // agar tidak bentrok dengan state di mobile
        onMouseEnter={() => {
          if (!isMobileOpen) {
      ...
      {/* --- PERUBAHAN 2 --- */}
      {/* Saat overlay diklik, reset kedua state */}
      {isMobileOpen && (
        <div 
      ...
      <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${isSidebarExpanded ? 'lg:ml-64' : 'lg:ml-20'}`}>
        <header className="bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            {/* --- PERUBAHAN 3 --- */}
            {/* Saat hamburger diklik, set kedua state ke true */}
            <button 
